...
public class InvalidRequestExceptionHandler extends ResponseEntityExceptionHandler{
    ...
    protected ResponseEntity<Object> handleAccessDeniedException(RuntimeException e, WebRequest request) {
        AccessDeniedException ire = (AccessDeniedException) e;

        ErrorResource error = new ErrorResource("Access is denied", ire.getMessage());
...
//        error.setEnglishErrorMessage(messageByLocaleService.getMessageEnglish(ire.getMessage()));
        ...
        return handleExceptionInternal(e, IOUtils.toString(microserviceResponse.getBody()), headers, microserviceResponse.getStatusCode(), request);
    }
